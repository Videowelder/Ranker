...
async function SRTable(guildID,user,channel,detail){
	...
	let del = new Discord.RichEmbed()
		...
		.addField("Total EXP:",exp | 0, true)
		.addField("To next level:",next | 0, true);
		if (detail) {
			del.addField("Message EXP:",(guilds[guildID][user.id]["msgs"]*15) | 0, true)
			del.addField("Reaction EXP:",(guilds[guildID][user.id]["rctn"]*5) | 0, true)
			del.addField("Voice EXP:",(guilds[guildID][user.id]["voce"]*10) | 0, true)
		}
...
async function MRTable(guildID,user1,user2,channel,detail){
	...
	let del = new Discord.RichEmbed()
		...
		.addField("Levels:",level, true)
		.addField("Total EXP:",Math.abs(exp1-exp2) | 0, true)
		if (detail) {
			del.addField("Message EXP:",Math.abs((guilds[guildID][user1.id]["msgs"]*15)-(guilds[guildID][user2.id]["msgs"]*15)) | 0, true)
			del.addField("Reaction EXP:",Math.abs((guilds[guildID][user1.id]["rctn"]*5)-(guilds[guildID][user2.id]["rctn"]*5)) | 0, true)
			del.addField("Voice EXP:",Math.abs((guilds[guildID][user1.id]["voce"]*10)-(guilds[guildID][user2.id]["voce"]*10)) | 0, true)
		}
	await channel.send(del)
...
bot.on('message', async (Message) => {
	...
	if (Message.cleanContent.startsWith(opts.prefix + "rank")) {
		...
		await Message.channel.send("Your timestamp has been reset");
		
	} else if (Message.content.match(/!estimate\s*lv(\d*)>/)) {
		
		await Message.channel.send("You are X exp away from level ");
		
	} else if (Message.cleanContent.startsWith(opts.prefix + "clear my ranking please and thank you!")) {
		...
		guilds[Message.guild.id][Message.author.id]["instmp"] = Date.now();
		await Message.channel.send("Ranking cleared and timestamp reset");
